 ...
 *
 * Setup on Arduino:
 * - Sensors can be added starting from pin 4, 5.
 * - The first pin is the input (echo) and second the output (trig).
 * - Sensors have ids 1 through n amounts of sensors.
 *
 * Example:
 * - Single sensor:  pins 4 & 5 is sensor with id "1".
 *
 * - Dual sensor:    pins 4 & 5 is sensor with id "1".
 *                   pins 6 & 7 is sensor with id "2".
 *
 * - Triple sensor:  pins 4 & 5 is sensor with id "1".
 *                   pins 6 & 7 is sensor with id "2".
 *                   pins 8 & 9 is sensor with id "3".
 */
...
void loop()
{
    ...
    for (int i = 0; i < SENSOR_COUNT; i++)
    {
        sensors[i] = isTriggered(i + 1, 5 + i * 2, 4 + i * 2);
    }
